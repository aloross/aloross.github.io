...
const WorkHistory = ({ history }) => {
  return (
    <>
      <h5 className="font-header font-semibold text-front text-base uppercase mt-12 mb-3 text-center">
        Expérience Pro
      </h5>
      <div className="history flex flex-col relative pt-6 pb-6">
        {history.map(({ company, period, position, url }, i) => (
          ...
          >
            ...
            {position && (
              <h5 className="text-base font-normal mb-1">{position}</h5>
            )}
            {period && (
              <span className="text-base font-medium opacity-50">{period}</span>
            )}
          </div>
        ))}
      </div>
